  ...
  const consumir_login = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        window.location = "/profile/"+localStorage.getItem('userIdLocal');
      })
      .catch((error) => {
        console.log(error.response.data);
        if(error.response.data.non_field_errors == null){
          if(error.response.data.password != null && error.response.data.username == null){
            errorMensaje = "Falta el campo Password";
          }else if(error.response.data.username[0] != null && error.response.data.password == null){
            errorMensaje = "Falta el campo Username"
          }else{
            errorMensaje = "Por favor rellena el formulario";
          }
        }else{
          errorMensaje = error.response.data.non_field_errors;
        }
        document.getElementById("message").textContent = errorMensaje;
      setTimeout(function(){
  ...
  return (
    <div className={StyleLogin.body}>
      <div className={StyleLogin.container}>
        <svg className={StyleLogin.onda} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320">
        ...
        <div className={StyleLogin.formContainer}>
          ...
          <h1>Iniciar sesión</h1>
            <div className={StyleLogin.group}>
              <input type="text" id="user" required/> <span className={StyleLogin.border_bottom}></span>
              <label>Nombre de usuario</label>
            </div>
            <div className={StyleLogin.group}>
              <input type="password" id="pass" required/> <span className={StyleLogin.border_bottom}></span>
              <label>Contraseña</label>
            </div>
            <p id="message"></p>
            <button type="submit" onClick={consumir_login}> Enviar </button>
          </div>
